...
\subsection{System Analysis and DoF}
\textbf{Generalized Coordinates}
\\
(Separated into unactuated base $\mathbf{q}_b$
and actuated joints $\mathbf{q}_j$)
\[
  \mathbf{q} =
  ...
  \text{dims:}
  \begin{cases}
    \text{Fixed Base:} & n = n_j \\
    \text{Planar (2D):} & n = 3 + n_j \\
    \text{Floating (3D):} & n = 6 + n_j
  \end{cases}
\]
\textbf{Contact Constraints} $n_c = \sum n_{c,i}$
\begin{itemize}
  \item \textbf{Point Contact} no-slip:
    $n_c=3$ (3D) or $n_c=2$ (2D) per foot
  \item \textbf{Surface Contact} $n_c=6$ (3D) per foot
\end{itemize}

\textbf{Mobility and Actuation Analysis}
\begin{itemize}
  \item \textbf{Uncontrollable DoF}
    $\dim(\mathbf{q}_b) - \text{rank}(\mathbf{J}_\text{contact})$\\
    (e.g., Planar base + 1 point contact: $3 - 2 = 1$ uncontrollable)
  \item \textbf{Degree of Underactuation}
    $\dim(\mathbf{q}_b) - n_c$\\
    (If $>0$, system is underactuated)
\end{itemize}

\textbf{System Mobility (Net DoF)}
$ \delta_{sys} = n_{total} - n_c $

Example: Quadruped, $n=18$
\begin{itemize}
  \item 3 Stance Legs (Point): $n_c = 3 \times 3 = 9$
  \item Net DoF: $18 - 9 = 9$
\end{itemize}
...
- Why torque control? Compliance for rough terrain
(vs. stiff position control). \\
